...
Some solvers can be run in a multithreaded mode.  For Solvers that support it, users can run in multithreaded mode simply by calling setNumThreads on the Solver.  For example
...
For large graphs are graphs with compute intensive factor tables, multithreading should speedup solve time close to N times, where N is the number of cores in the user's machine.  For medium size graphs or small graphs, the multithreading mode might not accelerate solving.
