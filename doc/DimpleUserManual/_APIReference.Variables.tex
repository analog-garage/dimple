...
\[
BentheEntropy(i) = - \sum{d \in D}B_i(d)*log(B_i(d))
\]
...
\item Real(new RealDomain(-Math.PI, Math.PI)) specifies a scalar real variable with domain from $-\pi$ to $\pi$.
\end{itemize}
\fi
...
\begin{lstlisting}
Complex()
Complex(RealJointDomain domain)
\end{lstlisting}

The arguments are defined as follows:

\begin{itemize}
\item domain specifies the domain of the Complex variable using a RealJointDomain object (see \ref{sec:RealJointDomain}).  Using this version of the constructor allows bounds to be specified in some or all dimensions of the domain.  For a Complex variable, the number of dimensions of the RealJointDomain must equal 2.
\end{itemize}
\fi
...
The RealJointDomain class is used to refer to the domain of RealJoint (or Complex) variables.
...
\begin{lstlisting}
domain.getRealDomains(dimensionIndex);
\end{lstlisting}
