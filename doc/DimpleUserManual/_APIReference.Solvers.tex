...
\item AUTOMATIC - Automatically determine whether to use the optimized algorithm based on attributes of the factor tables and the graph. The automatic selection algorithm can be tuned through the \nameref{option:SumProductOptions.automaticExecutionTimeScalingFactor} and \nameref{option:SumProductOptions.automaticMemoryAllocationScalingFactor} options.
\end{itemize}
...
{100.0}
{discrete factors}
{This option is an execution time scaling factor used when the \nameref{option:SumProductOptions.updateApproach} option is set to AUTOMATIC. It controls how execution time and memory allocation costs are weighed. The value must be a positive number.}
