...
function testConstMult()
    ...
    if isempty(which('gf'))
        dtrace(true, 'WARNING: testConstMult was skipped because Communications Toolbox not installed');
        return;
    end
    ...
    for constAsSecondParam = 0:1
        ...
        for yindex = 2:length(tmp)
            ...
            assertEqual(class(f.Solver),'com.analog.lyric.dimple.solvers.sumproduct.customFactors.CustomFiniteFieldConstantMult');
