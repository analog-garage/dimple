...
public class JunctionTreeTransform implements IFactorGraphTransform
{
	 ...
	 * <p>
	 * @param eliminationOrder is a valid variable ordering for this graph. Typically it will have been
	 * previously created by using a {@link VariableEliminator} on this {@code model}, but it
	 * may also be generated by a custom user-defined variable elimination ordering algorithm. It's
	 * {@code variables} list must contain exactly the same variables as the {@code model} with
	 * no duplicates. If the {@code stats} object returns true for
	 * {@link Stats#alreadyGoodForFastExactInference()}, then this method will simply return
	 * {@link FactorGraphTransformMap#identity(FactorGraph)} for the {@code model}. The
	 * {@code stats} object must also produce a correct value for
	 * {@link Stats#factorsWithDuplicateVariables()}.
	 */
