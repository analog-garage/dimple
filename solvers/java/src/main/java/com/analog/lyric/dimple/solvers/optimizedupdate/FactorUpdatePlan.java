...
public final class FactorUpdatePlan
{
	 ...
	 * @since 0.06
	 */
	public static FactorUpdatePlan create(final IFactorTable factorTable,
		final UpdateSettings factorTableOptimizationSettings,
		final ITableWrapperAdapter tableWrapperAdapter)
	{
		...
		ITreeBuilder<TableWrapper> builder =
			new TreeBuilder(steps, factorTableOptimizationSettings, tableWrapperAdapter);
	...
	private static class TreeBuilder implements ITreeBuilder<TableWrapper>
	{
		...
		public TreeBuilder(List<IUpdateStep> steps,
			UpdateSettings factorTableOptimizationSettings,
			final ITableWrapperAdapter tableWrapperHelper)
		{
			_steps = steps;
		...
		final ISFactorGraphToCostEstimationTableWrapperAdapter tableWrapperHelper)
	{
		...
		ITreeBuilder<CostEstimationTableWrapper> builder = new EstimationTreeBuilder(result, tableWrapperHelper);
	...
	private static class EstimationTreeBuilder implements ITreeBuilder<CostEstimationTableWrapper>
	{
		...
		public EstimationTreeBuilder(Costs result, ISFactorGraphToCostEstimationTableWrapperAdapter tableWrapperHelper)
		{
			_result = result;
			_tableWrapperHelper = tableWrapperHelper;
		}
		...
		{
			return new CostEstimationTableWrapper(rootFactorTable, _tableWrapperHelper);
		}
